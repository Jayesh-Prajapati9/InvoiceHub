...
const EmailModal = ({ 
  ...
  const [subject, setSubject] = useState(`Quote ${quoteNumber}`);
  const [message, setMessage] = useState(`Please find attached quote ${quoteNumber}.`);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTo(emailToUse);
      setSubject(`Quote ${quoteNumber}`);
      setMessage(`Please find attached quote ${quoteNumber}.`);
    }
  }, [isOpen, contactEmail, quoteNumber]);

  const sendEmailMutation = useMutation({
    mutationFn: async (data: { to: string; subject: string; message: string }) => {
      const res = await api.post(`/quotes/${quoteId}/send-email`, data);
      return res.data;
    },
    onSuccess: () => {
      onSuccess?.();
      onClose();
      showToast('Quote sent via email successfully', 'success');
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-gray-900">Send Quote via Email</h3>
                <button
                  type="button"
                  onClick={onClose}
