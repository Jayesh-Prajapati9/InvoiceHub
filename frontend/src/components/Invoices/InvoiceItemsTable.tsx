...
const InvoiceItemsTable = ({ fields, register, watch, setValue, items, onItemSelect }: InvoiceItemsTableProps) => {
  ...
  const calculateItemTotal = (item: InvoiceItem | any, index: number) => {
    // Headers don't have totals
        ...
        <div className="overflow-x-auto">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {fields.fields.map((field, index) => {
              ...
              const isReadOnly = isTimesheet;
              
              return (
                <tr key={field.id} className={`hover:bg-gray-50 transition-colors ${isHeader ? 'bg-gray-100' : ''} ${isReadOnly ? 'bg-gray-50' : ''}`}>
                  <td className="px-6 py-4" colSpan={isHeader ? 4 : 1}>
                    {isHeader ? (
                      <div>
                        <input
                          {...register(`items.${index}.name`)}
                          placeholder="Add New Header"
                          className="input-field text-sm w-full font-semibold text-gray-900 bg-transparent border-0 focus:ring-0 focus:border-b-2 focus:border-primary-500"
                        />
                      </div>
                    ) : (
